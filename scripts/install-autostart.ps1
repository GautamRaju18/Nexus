# Installs Nexus to start automatically when you log in (no admin required).
...
$vbs = Join-Path $scriptDir "nexus-hidden.vbs"

if (-not (Test-Path $vbs)) { throw "Launcher not found: $vbs" }

# Sanity: node must be on PATH for the logon launch to work.
$node = Get-Command node -ErrorAction SilentlyContinue
if (-not $node) { Write-Warning "node was not found on PATH. Nexus will not start at logon until Node.js is on PATH." }

$startup = [Environment]::GetFolderPath("Startup")
$lnk = Join-Path $startup "Nexus.lnk"
...
$sc.Description = "Nexus - AI Chief of Staff (web cockpit)"
$sc.WindowStyle = 7
$sc.Save()
...
Write-Host ""
Write-Host "  Nexus starts next time you log in and opens http://127.0.0.1:4321"
